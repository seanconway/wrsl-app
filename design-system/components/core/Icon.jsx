...
/* Inline SVG glyphs, drawn to the Lucide geometry the design system specifies:
   24px grid, 2px stroke, round caps and joins, stroke-only, always currentColor.

   LOCAL RATHER THAN CDN. The design system's readme substitutes Lucide loaded
   from unpkg. This application must run a complete match with the venue's
   network entirely absent (SCOPE.md §7.3), and an icon set fetched at first
   render is exactly the kind of dependency that works on the bench and fails
   at the event. The working set is small and fixed, so it is inlined here.

   To add a glyph: copy the path data from the Lucide source for that name and
   add it below. Do not reintroduce a network fetch. */
const GLYPHS = {
...
export function Icon({ name, size = 20, color, style, label, ...rest }) {
